...
export default function EditClientPage() {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      try {
        ...
        setServerError("Failed to load client data. Please try again later.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const validateForm = () => {
    ...
    if (formData.mobile && !/^\d{9,10}$/.test(formData.mobile.replace(/[-\s]/g, ''))) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "אירעה שגיאה בעת עדכון הלקוח");
      }
      ...
      setServerError(err.message || "אירעה שגיאה בעת עדכון הלקוח");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">עריכת לקוח</h1>
        <p className="text-gray-500">עדכן את פרטי הלקוח</p>
      </div>
